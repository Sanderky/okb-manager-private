...
export const VacationsCalendar: React.FC = () => {
  ...
  const { mutate: addMutation } = useMutation({
    mutationFn: (payload: Partial<Vacation>) => VacationApi.createVacation(payload),
  ...
  const { mutate: updateMutation } = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<Vacation> }) =>
      VacationApi.updateVacation(id, data),
    onMutate: () => startActionLoading(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vacations'] });
      queryClient.invalidateQueries({ queryKey: ['schedules'] });
  ...
  const { mutate: deleteMutation } = useMutation({
    mutationFn: (id: string) => VacationApi.removeVacation(id),
    onMutate: () => startActionLoading(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vacations'] });
      queryClient.invalidateQueries({ queryKey: ['schedules'] });
