...
interface AlertContextType {
  ...
  updateAlert: (id: string, updates: Partial<Omit<EmployeeAlert, 'id'>>) => void;
  resetAlerts: () => void
}

const EmployeeAlertContext = createContext<AlertContextType | undefined>(undefined);

export const EmployeeAlertProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const removeAlertsByEmployee = (employeeId: string) => {
    setAlerts(prev => prev.filter(alert => alert.employeeId !== employeeId));
  };
  ...
  const updateAlert = (id: string, updates: Partial<Omit<EmployeeAlert, 'id'>>) => {
    setAlerts(prev => 
      prev.map(alert => 
  ...
  return (
    <EmployeeAlertContext.Provider value={{ 
      alerts, 
      addAlert, 
      addAlertWithId,
      removeAlert, 
...
export const useEmployeeAlert = () => {
  ...
  if (context === undefined) {
    throw new Error('useEmployeeAlert must be used within an EmployeeAlertContext');
  }
  return context;
};