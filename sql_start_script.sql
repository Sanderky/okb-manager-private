...
create table lodgings (
  id uuid default gen_random_uuid() primary key,
  created_at timestamptz default now(),
  name text,        -- Zmieniono na opcjonalne (nullable)
  address text,     -- Zmieniono na opcjonalne (nullable)
  description text,
  start_date date not null,
  end_date date not null,
  construction_site_id uuid references constructions(id) on delete set null, -- Dodano powiązanie z budową
...
create table lodging_employees (
  ...
  employee_id uuid references employees(id) on delete cascade not null,
  start_date date, -- Dodano datę od dla konkretnego pracownika
  end_date date,   -- Dodano datę do dla konkretnego pracownika
...
create policy "Auth All" on employees for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on contractors for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on constructions for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on vacations for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on daily_schedules for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on work_logs for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on alert_settings for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on home_notes for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on calendar_events for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on calendar_event_employees for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on calendar_event_constructions for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on lodgings for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on lodging_employees for all using ((select auth.role()) = 'authenticated');
create policy "Auth All" on todos for all using ((select auth.role()) = 'authenticated');
